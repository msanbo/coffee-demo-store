...
// Requests a password reset email. The emailpass provider's update route
// only accepts single-use tokens issued by this flow (see resetPassword
// below) - it rejects an ordinary logged-in session token - so changing a
// password always goes through this email round trip, even from the
...
// Completes a password reset using the single-use token from the email sent
// by requestPasswordReset. The token itself (not a logged-in session) is
// what authorizes this call.
